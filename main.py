...
TOKEN = "RUB"
BOT_TOKEN = "5703266677:AAFLORtoyhQw5GrM-EGblKC7smR-R711cuE"
OWNER_ID = 1042554792
...
def start(message):
    ...
    if startms == st:
        db = sqlite3.connect('users.db')
        sql = db.cursor()
        sql.execute("SELECT id FROM users WHERE id = ?", [userr])
        values = [userr, user_namess]
        if sql.fetchone() is None:
            sql.execute("INSERT INTO users(id, user_name) VALUES(?, ?)", values)
            db.commit()
        try:
            ...
            db = sqlite3.connect('users.db')
            sql = db.cursor()
            sql.execute("SELECT parent_id FROM users WHERE id = ?", [userr])
            if sql.fetchone()[0] is None:
                sql.execute("UPDATE users SET parent_id = ? WHERE id = ?", [ref_id, userr])
                db.commit()
        except:
            userr = message.chat.id
            db = sqlite3.connect('users.db')
            ...
            sql.execute("SELECT parent_id FROM users WHERE id = ?", [userr])
            if sql.fetchone()[0] is None:
                sql.execute("UPDATE users SET parent_id = ? WHERE id = ?", [id_no_ref, userr])
                db.commit()
        ...
        if sql.fetchone() is None:
            sql.execute("INSERT INTO users(id, user_name, parent_id) VALUES(?, ?, ?)", values)
            db.commit()
    ...
    try:
        ...
        if call.data == 'check':
            if ch == True:
                ...
                db = sqlite3.connect('users.db')
                sql = db.cursor()
                sql.execute("SELECT sub_chek FROM users WHERE id = ?", [user])
                if sql.fetchone()[0] == 0:
                    parent_id = sql.execute("SELECT parent_id FROM users WHERE id = ?", [user]).fetchone()[0]
                    parent_id2 = sql.execute("SELECT parent_id FROM users WHERE id = ?", [parent_id]).fetchone()[0]
                    bal_par = sql.execute("SELECT balance FROM users WHERE id = ?", [parent_id]).fetchone()[0]
                    ref1 = sql.execute("SELECT ref1 FROM users WHERE id = ?", [parent_id]).fetchone()[0]
                    ref2 = sql.execute("SELECT ref2 FROM users WHERE id = ?", [parent_id2]).fetchone()[0]
                    r1 = ref1 + 1
                    r2 = ref2 + 1
                    bal = bal_par + per_ref
                    sql.execute("UPDATE users SET sub_chek = ? WHERE id = ?",[1, user])
                    sql.execute("UPDATE users SET ref1 = ? WHERE id = ?", [r1, parent_id])
                    sql.execute("UPDATE users SET ref2 = ? WHERE id = ?", [r2, parent_id2])
                    sql.execute("UPDATE users SET sub_chek = ? WHERE id = ?", [1, user])
                    sql.execute("UPDATE users SET balance = ? WHERE id = ?", [bal, parent_id])
                    db.commit()
...
def send_text(message):
   try:
    if message.text == '🆔 Аккаунт':
        ...
        accmsg = '*👮 Пользователь : {}\n\n⚙️ Кошелёк : *`{}`*\n\n💸 Баланс : *`{}`* {}*'
        user_id = message.chat.id
        balance = sql.execute("SELECT balance FROM users WHERE id = ?", [user]).fetchone()[0]
        wallet = sql.execute("SELECT wallet FROM users WHERE id = ?", [user]).fetchone()[0]
    ...
    if message.text == '🙌🏻 Рефералы':
        ...
        db = sqlite3.connect('users.db')
        sql = db.cursor()
        ref_count = sql.execute("SELECT ref1 FROM users WHERE id = ?", [user]).fetchone()[0]
    ...
    def trx_address(message):
        try:
            ...
            if len(message.text) > 8:
                ...
                user = str(user_id)
                wallet = message.text
                sql.execute("UPDATE users SET wallet = ? WHERE id = ?", [wallet, user])
                wal1 = sql.execute("SELECT wallet FROM users WHERE id = ?", [user]).fetchone()[0]
                db.commit()
    ...
    def amo_with(message):
        try:
            ...
            db = sqlite3.connect('users.db')
            sql = db.cursor()
            bal = sql.execute("SELECT balance FROM users WHERE id = ?", [user]).fetchone()[0]
            wall = sql.execute("SELECT wallet FROM users WHERE id = ?", [user]).fetchone()[0]
            ...
            amo = int(amo)
            ball = bal - int(amo)
            sql.execute("UPDATE users SET balance = ? WHERE id = ?", [ball, user])
            db.commit()
